...
## Rol
Especialista en schema SQL, migrations, RLS y optimización de queries para SkuBeam. Todo el schema está diseñado para multi-tenancy por `shop_id`.

## Cuándo invocarme
- Crear o modificar tablas
- Diseñar RLS policies
- Optimizar queries lentas
- Crear la migration inicial del proyecto
- Generar tipos TypeScript desde el schema

---

## Migration 001 — Schema completo de SkuBeam
...
-- ============================================
-- SESIONES SHOPIFY (reemplaza Prisma/SQLite)
-- ============================================
...
-- ============================================
-- VISTA: SKU ANALYTICS (se refresca con cron)
-- ============================================
...
-- ============================================
-- FUNCIÓN: refresh analytics (llamar con cron)
-- ============================================
...
# Generar tipos TypeScript (correr después de cada migration)
...
## Checklist de seguridad por tabla

| Tabla | RLS | Policy | Service Role accede |
|---|---|---|---|
| shopify_sessions | ✅ | Sin policy pública | ✅ Solo server |
| shops | ✅ | shop_id isolation | ✅ |
| skus | ✅ | shop_id isolation | ✅ |
| inventory_levels | ✅ | shop_id isolation | ✅ |
| sales_history | ✅ | shop_id isolation | ✅ |
| sync_jobs | ✅ | shop_id isolation | ✅ |
| forecast_configs | ✅ | shop_id isolation | ✅ |
| gdpr_requests | ✅ | Sin policy pública | ✅ Solo server |

**Regla de oro**: en SkuBeam usamos `supabaseAdmin` (service_role) en el servidor y filtramos `shop_id` explícitamente en cada query. RLS es la segunda línea de defensa.
